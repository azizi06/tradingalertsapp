...
class TimerBackgroundTaskManager {
  late Timer _timer;
      ...
      try {
        ...
        for (AlertModel alert in snapshot) {
          ...
          if (coinID != null && value != null) {
            final data = await getCoinPriceById(coinID); //fetchPrice(coinID);

            if (data != null) {
              final price = data['price'];
              final change24H = data['price_change_percentage_24h'];
              print(
                  "CoinID : $coinID ,Type , $alert.t Value:  $value ,Price  : $price  ");
              switch (alert.type) {
                case "priceOver":
                  if (price > value) {
                    print(
                ...
                case AlertType.priceBelow:
                  ...
                  if (alert.documentId != null) {
                    print(
                        " $coinID price is below \$${value}. Current price: \$${price} Documment ID is null ");
                    alertService.updateIsNotified(alert.documentId ?? "");
                  }
                  break;
...
Future<Map<String, dynamic>?> fetchPrice(String coinID) async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      return getCoinPriceById(coinID);
    }
  } catch (e) {
